                            ...
                            with get_session() as session:
                                ...
                                if not profile:
                                    ...
                                    if new_password and len(new_password) >= 6:
                                        ...
                                        st.rerun()
                                elif profile.password_hash == hash_password(login_password):
                                    # Successful login
                                    st.session_state.user_profile = profile.to_dict()
                                    st.session_state.user_email = login_email
...
if uploaded_files:
    ...
    with col2:
        if st.button("🚀 Extract with Gemini", use_container_width=True, type="primary"):
            ...
            try:
                status_text.info("🔄 Analyzing documents with AI...")
                ...
                status_text.success(f"✅ Extraction completed in {elapsed:.1f}s")
                time.sleep(1)
                status_text.empty()
                progress_bar.empty()
                
            except GeminiExtractionError as exc:
                status_text.warning(f"⚠️ Gemini failed, trying OCR...")
                ...
                try:
                    ...
                    status_text.info("✓ OCR completed")
                except Exception as e:
                    status_text.error(f"❌ Extraction failed: {e}")
                    st.stop()
                finally:
                    if 'tmp' in locals():
    ...
    try:
        with get_session() as session:
            ...
            for idx, user in enumerate(filtered_users):
                with st.expander(
                    f"👤 {user.user_email} - {user.full_name or 'No name'}",
                    expanded=False
                ):
                    ...
                    with col1:
                        ...
                        st.write(f"**Email (Contact):** {user.email or 'Not set'}")
                    ...
                    with col2:
                        ...
                        # Update button
                        if st.button("✏️ Update", key=f"update_{user.id}", use_container_width=True):
                            st.session_state[f"editing_user_{user.id}"] = True
                            st.rerun()
                        
                        # Delete button
                        if st.button("🗑️ Delete", key=f"delete_{user.id}", use_container_width=True, type="secondary"):
                            st.session_state[f"confirm_delete_{user.id}"] = True
                    
                    with col3:
                        st.markdown("**View:**")
                        if st.button("👁️ View Full", key=f"view_{user.id}", use_container_width=True):
                            st.session_state[f"viewing_user_{user.id}"] = True
                    
                    # Delete confirmation
                    if st.session_state.get(f"confirm_delete_{user.id}", False):
                        st.warning(f"⚠️ Are you sure you want to delete user: {user.user_email}?")
                        col_del1, col_del2 = st.columns(2)
                        with col_del1:
                            if st.button("✅ Confirm Delete", key=f"confirm_del_{user.id}", use_container_width=True, type="primary"):
                                try:
                                    session.delete(user)
                                    session.commit()
                    ...
                    if st.session_state.get(f"editing_user_{user.id}", False):
                        ...
                        with st.form(key=f"edit_user_{user.id}"):
                            ...
                            updated_address = st.text_area("Address", value=user.permanent_address or "", key=f"edit_address_{user.id}")
                                    ...
                                    try:
                                        ...
                                        if updated_mobile:
                                            ...
                                            if not phone_valid:
                                                ...
                                                user.mobile_number = updated_mobile
                                        ...
                                        if updated_contact_email:
                                            ...
                                            if not email_valid:
                                                ...
                                                user.email = updated_contact_email
                                        
                                        user.full_name = updated_name or None
                                        user.father_name = updated_father or None
                                        user.permanent_address = updated_address or None
                                        user.business_name = updated_business or None
                    ...
                    if st.session_state.get(f"viewing_user_{user.id}", False):
                        st.markdown("---")
                        st.markdown("**👁️ Full User Profile:**")
